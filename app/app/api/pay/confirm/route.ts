 ...
 *
 * The client sends a signature. That is a claim, not evidence: the transaction
 * is fetched from the chain and checked against the intent the server recorded
 * (right payer, right recipient, exact amount), and only then is the entry or
 * the practice balance credited.
 *
...
export async function POST(request: Request): Promise<Response> {
  ...
  // How much practice balance a store purchase buys, recomputed from the price
  // rather than taken from the request. Entry has no such credit.
  const tier =
